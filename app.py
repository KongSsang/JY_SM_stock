...
krw_balance = 0
usd_purchases = []
krw_row_idx = 2 # 기본값 (KRW가 2행에 있다고 가정)
...
with tab2:
    ...
    if form_type == "💰 원화 입출금":
        with st.form("krw_log_form", clear_on_submit=True):
            st.subheader("원화 입출금 기록")
            c1, c2 = st.columns(2)
            ...
            c3, c4 = st.columns(2)
            log_amount = c3.number_input("변동 금액 (원)", step=10000, key="k_amt")
            log_memo = c4.text_input("상세 내용", placeholder="예: 4월 월급 입금", key="k_memo")
            
            submitted = st.form_submit_button("원화 변동 기록하기")
            if submitted:
                if "입금" in inout_type:
                    new_krw = krw_balance + log_amount
                else:
                    new_krw = krw_balance - log_amount
                    
                sheet_cash.update_cell(krw_row_idx, 2, new_krw)
                sheet_log.append_row([str(log_date), inout_type, log_amount, log_memo])
                st.success(f"성공! 원화 잔고가 {new_krw:,.0f}원으로 업데이트되었습니다.")
                st.rerun()

    elif form_type == "💵 달러 환전":
        with st.form("usd_log_form", clear_on_submit=True):
            st.subheader("달러 환전 기록")
            st.markdown("💡 *입력하신 원화 금액만큼 KRW 잔고에서 차감되고, USD 잔고에 추가됩니다.*")
            
            c1, c2 = st.columns(2)
            usd_date = c1.date_input("환전 날짜", datetime.now(pytz.timezone('Asia/Seoul')), key="u_date")
            usd_amount = c2.number_input("환전된 달러 (USD)", step=100.0, format="%.2f", key="u_amt")
            
            c3, c4 = st.columns(2)
            exch_rate = c3.number_input("적용 환율 (원/달러)", step=1.0, format="%.2f", key="u_rate")
            usd_memo = c4.text_input("상세 내용", placeholder="예: 여행 대비 환전", key="u_memo")
            
            submit_usd = st.form_submit_button("환전 기록하기")
            if submit_usd:
                krw_spent = usd_amount * exch_rate
                new_krw = krw_balance - krw_spent
                
                sheet_cash.update_cell(krw_row_idx, 2, new_krw)
                sheet_cash.append_row(['USD', usd_amount, exch_rate])
                
                log_memo_text = f"{usd_amount}달러 환전 (@{exch_rate:,.2f}) - {usd_memo}"
                sheet_log.append_row([str(usd_date), "달러 환전", krw_spent, log_memo_text])
                
                st.success(f"성공! 환전 비용 {krw_spent:,.0f}원이 KRW 잔고에서 차감되었습니다.")
                st.rerun()

    else: 
        with st.form("stock_buy_form", clear_on_submit=True):
            st.subheader("새로운 주식 매수 기록하기")
            st.markdown("💡 *매수 총액만큼 KRW 잔고에서 자동으로 차감됩니다.*")
            ...
            if submit_stock:
                if not buy_name or not buy_ticker:
                    ...
                    new_krw = krw_balance - total_buy_amount
                    ...
                    st.success(f"성공! {action_msg} (KRW 잔고에서 {total_buy_amount:,.0f}원 차감됨)")
                    st.rerun()
...
with tab3:
    ...
    with st.form("date_form", clear_on_submit=True):
        ...
        date_memo = c4.text_input("상세 내용", placeholder="예: 영화 예매 등", key="date_memo")
